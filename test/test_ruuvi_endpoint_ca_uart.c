...
void test_ruuvi_endpoint_ca_uart_decode_filter_ruuvi (void)
{
    ...
    re_ca_uart_ble_filter_t expect_params = { .manufacturer_id = 0x0499};
    re_ca_uart_cmd_t expect_cmd = RE_CA_UART_SET_FLTR;
    re_ca_uart_ble_filter_t params = { .manufacturer_id = 0};
    re_ca_uart_cmd_t cmd = 0;
    err_code = re_ca_uart_decode (data, &cmd, &params);
    TEST_ASSERT (RE_SUCCESS == err_code);
    TEST_ASSERT (!memcmp (&expect_params, &params, sizeof (params)));
    TEST_ASSERT (!memcmp (&expect_cmd, &cmd, sizeof (cmd)));
}
...
void test_ruuvi_endpoint_ca_uart_decode_null (void)
{
    ...
    re_ca_uart_ble_filter_t params = { .manufacturer_id = RE_CA_UART_BLE_NOFILTER};
    uint8_t buffer[RE_CA_UART_TX_MAX_LEN] = {0};
    err_code = re_ca_uart_decode (NULL, &cmd, &params);
    TEST_ASSERT (RE_ERROR_NULL == err_code);
    err_code = re_ca_uart_decode (buffer, NULL, &params);
    TEST_ASSERT (RE_ERROR_NULL == err_code);
    err_code = re_ca_uart_decode (buffer, &cmd, NULL);
    TEST_ASSERT (RE_ERROR_NULL == err_code);
}
...
void test_ruuvi_endpoint_ca_uart_adv_decode (void)
{
    ...
    re_ca_uart_cmd_t cmd = 0;
    re_ca_uart_ble_scan_t params = { 0 };
    err_code = re_ca_uart_decode (data, &cmd, &params);
    TEST_ASSERT (RE_SUCCESS == err_code);
    TEST_ASSERT (!memcmp (&expect_params, &params, sizeof (params)));
    TEST_ASSERT (!memcmp (&expect_cmd, &cmd, sizeof (cmd)));
