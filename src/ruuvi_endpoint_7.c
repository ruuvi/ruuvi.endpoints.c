...
re_7_decode_temperature(const uint8_t* const buffer)
{
    ...
    coded_temperature |= ((uint16_t)buffer[RE_7_OFFSET_TEMP_MSB] & RE_BYTE_MASK) << RE_BYTE_1_SHIFT;
...
re_7_decode_humidity(const uint8_t* const buffer)
{
    ...
    coded_humidity |= ((uint16_t)buffer[RE_7_OFFSET_HUMI_MSB] & RE_BYTE_MASK) << RE_BYTE_1_SHIFT;
...
re_7_decode_pressure(const uint8_t* const buffer)
{
    ...
    coded_pressure |= ((uint16_t)buffer[RE_7_OFFSET_PRES_MSB] & RE_BYTE_MASK) << RE_BYTE_1_SHIFT;
...
re_7_decode_luminosity(const uint8_t* const buffer)
{
    ...
    coded_luminosity |= ((uint16_t)buffer[RE_7_OFFSET_LUMI_MSB] & RE_BYTE_MASK) << RE_BYTE_1_SHIFT;
...
re_7_encode_color_temp(uint8_t* const buffer, const re_7_data_t* data)
{
    ...
    {
        coded_color_temp = (uint8_t)((data->color_temp_k - RE_7_COLOR_TEMP_OFFSET) / RE_7_COLOR_TEMP_STEP);
    }
...
static void
re_7_decode_batt_motion(const uint8_t* const buffer, re_float* battery_v, uint8_t* motion_intensity)
{
...
re_7_check_format(const uint8_t* const p_buffer)
{
    ...
    if (RE_7_RAW_PACKET_ADV_DATA_TYPE_LEN_VAL != p_buffer[RE_7_RAW_PACKET_ADV_DATA_TYPE_LEN_OFFSET])
    {
        return false;
    }

    if (RE_7_RAW_PACKET_ADV_DATA_TYPE_FLAG1_VAL != p_buffer[RE_7_RAW_PACKET_ADV_DATA_TYPE_FLAG1_OFFSET])
    {
        return false;
    }
    ...
    const uint16_t manufacturer_id = (uint16_t)((uint16_t)p_buffer[RE_7_RAW_PACKET_MANUFACTURER_ID_OFFSET_HI]
                                                << RE_BYTE_1_SHIFT)
