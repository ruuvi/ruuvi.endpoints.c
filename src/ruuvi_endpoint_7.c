...
static void re_7_encode_sequence (uint8_t * const buffer, const re_7_data_t * data)
{
...
static uint8_t re_7_decode_sequence (const uint8_t * const buffer)
{
    return buffer[RE_7_OFFSET_SEQ];
}

static void re_7_encode_flags (uint8_t * const buffer, const re_7_data_t * data)
{
    uint8_t flags = 0;
...
static void re_7_decode_flags (const uint8_t * const buffer, re_7_data_t * const p_data)
{
...
static void re_7_encode_temperature (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (temperature))
    {
        ...
        int16_t rounded_temperature = (int16_t) roundf (temperature * RE_7_TEMP_RATIO);
...
static re_float re_7_decode_temperature (const uint8_t * const buffer)
{
...
static void re_7_encode_humidity (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (humidity))
    {
        ...
        coded_humidity = (uint16_t) roundf (humidity * RE_7_HUMI_RATIO);
    }
...
static re_float re_7_decode_humidity (const uint8_t * const buffer)
{
...
static void re_7_encode_pressure (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (pressure))
    {
        ...
        coded_pressure = (uint16_t) roundf (pressure * RE_7_PRES_RATIO);
    }
...
static re_float re_7_decode_pressure (const uint8_t * const buffer)
{
...
static void re_7_encode_tilt (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (acc_x) && !isnan (acc_y) && !isnan (acc_z))
    {
        ...
        if (magnitude > 0.001f)
        {
            ...
            coded_tilt_x = (int8_t) roundf (tilt_x * RE_7_TILT_RATIO);
            coded_tilt_y = (int8_t) roundf (tilt_y * RE_7_TILT_RATIO);
        }
    }
...
static void re_7_decode_tilt (const uint8_t * const buffer, re_float * tilt_x,
                              re_float * tilt_y)
{
...
static void re_7_encode_luminosity (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (luminosity))
    {
        ...
        coded_luminosity = (uint16_t) roundf (luminosity);
    }
...
static re_float re_7_decode_luminosity (const uint8_t * const buffer)
{
...
static void re_7_encode_color_temp (uint8_t * const buffer, const re_7_data_t * data)
{
    uint8_t coded_color_temp = RE_7_INVALID_COLOR_TEMP;

    if (data->color_temp_k >= RE_7_COLOR_TEMP_MIN && data->color_temp_k <= RE_7_COLOR_TEMP_MAX)
    {
...
static uint16_t re_7_decode_color_temp (const uint8_t * const buffer)
{
...
static void re_7_encode_batt_motion (uint8_t * const buffer, const re_7_data_t * data)
{
    ...
    if (!isnan (voltage))
    {
        ...
        coded_battery = (uint8_t) roundf ( (voltage - RE_7_BATT_OFFSET) * RE_7_BATT_RATIO);
...
static void re_7_decode_batt_motion (const uint8_t * const buffer, re_float * battery_v,
...
static void re_7_encode_motion_count (uint8_t * const buffer, const re_7_data_t * data)
{
...
static uint8_t re_7_decode_motion_count (const uint8_t * const buffer)
{
    return buffer[RE_7_OFFSET_MOTION_CNT];
}

static void re_7_encode_mac (uint8_t * const buffer, const re_7_data_t * data)
{
...
static uint64_t re_7_decode_mac (const uint8_t * const buffer)
{
    uint64_t mac = 0;
...
bool re_7_check_format (const uint8_t * const p_buffer)
{
    ...
    const uint16_t manufacturer_id = (uint16_t) ( (uint16_t)
                                     p_buffer[RE_7_RAW_PACKET_MANUFACTURER_ID_OFFSET_HI] << RE_BYTE_1_SHIFT)
