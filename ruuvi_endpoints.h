...
typedef enum{
  ...
  // endpoints 0x50 ... 0x5F are reserved for dsp handlers, however they're not enumerated but rather called dynamically
...
/**
 * Allowed sample rates. Unenumerated values are in Hz, i.e. 128 would be 128 Hz
 */
typedef enum {
  SAMPLE_RATE_STOP      = 0,   //Stop sampling
  SAMPLE_RATE_SINGLE    = 251, //Take a single sample
  SAMPLE_RATE_MIN       = 252, //Minimum
  SAMPLE_RATE_MAX       = 253, //Maximum
  SAMPLE_RATE_NO_CHANGE = 255
}ruuvi_samplerate_t;

/**
 * Rate at which sensor should send data to program.
 * Not implemented, poll for now.
 */
typedef enum {
...
/**
 * Where sensor should send it's data. i.e. sensor configured to 
 * transmit once per second to BLE_ADV should place a data tx into BLE ADV queue once per second
 */
typedef enum {
...
ret_code_t route_message(const ruuvi_standard_message_t* message);
